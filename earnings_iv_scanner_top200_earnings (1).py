...
@st.cache_data
def get_top_200_liquid_stocks():
    table = pd.read_html("https://etfdb.com/screener/?s=volume&f=asset_class:equity")[0]
    tickers = table[['Symbol', 'Volume']]
    tickers.columns = ['Ticker', 'Volume']
    tickers = tickers[tickers['Ticker'].str.isalpha()]
    return tickers.sort_values(by='Volume', ascending=False).head(200)['Ticker'].tolist()

@st.cache_data
def get_earnings_calendar():
    today = datetime.today().date()
    end = today + timedelta(days=14)
    tickers = get_top_200_liquid_stocks()
    earnings_info = []
        ...
        try:
            ...
            if not cal.empty:
                ...
                if isinstance(earn_date, pd.Timestamp):
                    ...
                    if today <= earn_date <= end:
                        earnings_info.append((ticker, earn_date))
        except:
            continue
    df = pd.DataFrame(earnings_info, columns=['Ticker', 'Earnings Date'])
    return df.sort_values(by='Earnings Date')
...
def run_app():
    st.title("Earnings IV Scanner (Top 200 Liquid + Earnings Within 14 Days)")
    earnings_df = get_earnings_calendar()
    st.subheader("Earnings Calendar (Next 2 Weeks)")
    st.dataframe(earnings_df)

    tickers = earnings_df['Ticker'].tolist()
    all_metrics = []
    all_plots = []
    for ticker in tickers:
    ...
    if all_metrics:
        ...
        st.warning("No valid data retrieved for the earnings window tickers.")
